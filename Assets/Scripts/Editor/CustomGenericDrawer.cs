...
[CustomPropertyDrawer(typeof(IDrawable), true)]
public class CustomGenericDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select"), FocusType.Keyboard))
        {
            ...
            {
                menu.AddDisabledItem(new GUIContent("Unavailable"));
                menu.ShowAsContext();
                return;
            }
...
[Serializable]
public class CustomDrawable : IDrawable { public int blaba = 0; }