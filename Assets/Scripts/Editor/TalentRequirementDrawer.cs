...
[CustomPropertyDrawer(typeof(ITalentRequirement), true)]
public class TalentRequirementDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Talent requirement Type"), FocusType.Keyboard))
        {
            var menu = new GenericMenu();
            if (typeMap == null || typeMap.Count == 0)
            {
                menu.AddDisabledItem(new GUIContent("No Talent requirements available"));
                menu.ShowAsContext();
                return;
            }
    ...
    {
        var baseType = typeof(ITalentRequirement);
